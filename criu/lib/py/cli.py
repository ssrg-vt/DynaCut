...
def remove_init_drio(opts):
    ...
    for p in ps_img['entries']:
        if(p['ppid'] == 0):
            pycriu.remove_init.remove_init_drio(directory, int(start_address, 16), get_task_id(p, 'pid'), trap_locations_filepath)
...
def config_init_drio(opts):
    ...
    for p in ps_img['entries']:
        if(p['ppid'] == 0):
            pycriu.remove_init.config_remove_init(directory, get_task_id(p, 'pid'), int(start_address, 16),\
...
def main():
    ...
    ci_parser.add_argument('-sa','--startaddress', help='VMA start address')
